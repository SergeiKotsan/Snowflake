...
JOIN ETRM_PUBLIC.HOURLYPROFILEHISTORY h ON c."profile_id" = h."PROFILE_ID"
...
WHERE 
    ...
    AND p."end_date" >= DATE_TRUNC('month', CURRENT_DATE())::DATE
    AND c."end_date" >= DATE_TRUNC('month', CURRENT_DATE())::DATE
    AND h."DATEX" >= c."start_date"
    AND h."DATEX" <= c."end_date"
    AND h."DATEX" >= DATE_TRUNC('month', CURRENT_DATE())::DATE
    AND h."PROFILE_ID" NOT IN (1,2,37)
    AND TRIM(p2."product") NOT IN ('Energy','Storage')
    AND TRIM(p2."product_type") <> 'RA'
    AND h."FILE_DATE"='2024-05-16'
 ;